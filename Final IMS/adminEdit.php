...
if(isset($_POST['profile'])){
    ...
    $updUsr = "UPDATE `users` SET `firstname`='".$fname."',`lastname`='".$lname."',`phn`='".$phn."',`address`='".$address."',`country`='".$country."' WHERE `id`='".$userId."'";
    $updRes = $conn->query($updUsr);
    if($updRes){
        $updated= '<p style="color:green;font-size:26px;">Updated successfully</p>';
    }else{
...
*/
    ...
    if(move_uploaded_file($tmp_name,$tarDir)){
        $updImag = "UPDATE `users` SET`img`='".$tarDir."' WHERE `id` ='".$userId."'";
        $ressImg = $conn->query($updImag);
        $uplIMgSUc = 'Uploaded images successfully';
    }else{
        echo "errr while uploading images";
    }
}
?>
                ...
                <div class="p-3 py-5">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div class="d-flex flex-row align-items-center back"><i class="fa fa-long-arrow-left mr-1 mb-1"></i>
                            <h6><a href="allUsers.php"><i style="font-size: 30px;" class="bi bi-house-door"></i>Back</a></h6>
                        </div>
