    ...
    if($fnameErr || $lnameErr || $passErr || $confirmErr || $emailErr!=''){
          ...
          $selRes = $conn->query($emailfind);
              ...
              if($selRes->num_rows > 0){
                ...
                if($insRes){
                  $dataRecorded='<p style="color:green;">data has been recorded successfully !</p>';
                }else{
...
//sign Up end...............................
  ...
  if(isset($_POST['login'])){
    ...
    $selLoginUser = "SELECT * FROM `users` WHERE `email`='".$loginemail."' AND `password`='".$loginpassword."'";

    $loginUserMailResult = $conn->query($selLoginUser);

    if($loginUserMailResult->num_rows >0){
      echo "data match you can login";
    }else{
